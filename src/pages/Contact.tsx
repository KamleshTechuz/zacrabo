...
import { ContHandChange, FormErrorKey, InitFormData } from "../shared/interfaces";

const initFormData: InitFormData = {name: '', email: '', num: '', msg: ''};
...
export const Contact = () => {
  ...
  const handleChange = ({target : { name, value}}: ContHandChange) => setFormData((pre) => ({...pre, [name]: value}));

  const checkError = (form: InitFormData) => {
    const errors: InitFormData&FormErrorKey = {
      name: onlyChars(form), email: emailValidator(form),
      num: checkNumber(form), msg: checkMsg(form),
    }
    setErrData((pre: any) => ({...pre, ...errors}));
  ...
  const sendEmail = (e: any) => {
    ...
    return (
      <div>
        <Modal isOpen={showLoader} onRequestClose={() => setShowLoader(false)} contentLabel="Example Modal" className="model">
